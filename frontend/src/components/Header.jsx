...
export default function Header({ currentView, setCurrentView, onOpenArchModal }) {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-white/10 bg-[#070A12]/85 backdrop-blur-2xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          
          {/* Brand Logo & Live Engine Badge */}
          <div className="flex items-center gap-3.5 cursor-pointer select-none" onClick={() => handleNav('intake')}>
            <div className="relative flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-tr from-brand-500 via-brand-400 to-aws-orange p-[1.5px] shadow-glow-teal transition-transform hover:scale-105">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 text-[10px] font-mono font-bold tracking-wider uppercase bg-brand-500/15 text-brand-300 border border-brand-500/30 rounded-full">
                  AI Agents
                </span>
              </div>
          ...
          <div className="hidden lg:flex items-center gap-2.5 bg-surface-900/80 px-4 py-1.5 rounded-full border border-white/10 text-xs shadow-inner">
            <span className="flex items-center gap-1.5 text-aws-orange font-semibold">
              <Zap className="w-3.5 h-3.5 fill-aws-orange" /> Step Functions
            </span>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              className="px-3 py-2 text-xs font-semibold rounded-xl text-slate-200 hover:text-white bg-surface-800/90 hover:bg-surface-750 border border-white/10 hover:border-aws-orange/40 flex items-center gap-1.5 transition-all shadow-sm"
              title="Inspect AWS Cloud Architecture"
            >
              <Layers className="w-4 h-4 text-aws-orange" />
              <span className="hidden md:inline">AWS Blueprint</span>
            </button>
            ...
            <button
              ...
              className={`px-3 sm:px-3.5 py-2 text-xs sm:text-sm font-semibold rounded-xl transition-all duration-200 flex items-center gap-1.5 ${
                currentView === 'dashboard'
                  ? 'bg-surface-700 text-white border border-brand-500/40 shadow-inner'
            ...
            <button
              ...
              className={`px-3.5 sm:px-4 py-2 text-xs sm:text-sm font-bold rounded-xl transition-all duration-200 flex items-center gap-1.5 ${
                currentView === 'intake' || currentView === 'stepper' || currentView === 'result'
                  ? 'bg-gradient-to-r from-brand-500 to-teal-400 text-surface-950 shadow-glow-teal hover:opacity-95'
